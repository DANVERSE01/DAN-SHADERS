  ...
  return (
    <section className="relative z-20 w-full py-20 overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full opacity-[0.03] pointer-events-none select-none overflow-hidden">
             <div className="whitespace-nowrap text-[20vw] font-bold text-[var(--text-main)] font-manrope animate-[infinite-scroll_60s_linear_infinite]">
        ...
        <div className="max-w-[1400px] mx-auto px-6 relative z-10">
            <div className="flex items-center justify-between mb-20 animate-on-scroll">
                <h2 className="text-4xl md:text-5xl font-medium text-[var(--text-main)] tracking-tight font-manrope">
                    SELECTED <span className="text-[var(--primary-500)]">WORKS</span>
                </h2>
                <a href="#" className="hidden md:flex items-center gap-2 text-sm uppercase font-bold tracking-widest text-[var(--text-dim)] hover:text-white transition-colors focus-visible:outline-none focus-visible:text-[var(--primary-500)]">
        ...
        <div className={`group relative flex flex-col md:flex-row ${align === 'right' ? 'md:justify-end' : ''} items-center animate-on-scroll`}>
            ...
            <div className={`w-full md:w-1/3 mb-10 md:mb-0 ${align === 'right' ? 'md:order-1 md:pr-12 md:text-right' : 'md:order-2 md:pl-12'}`}>
                <div className={`flex flex-col ${align === 'right' ? 'items-end' : 'items-start'}`}>
                    <span className="text-[var(--primary-500)] font-mono text-xs mb-4">/ {year}</span>
                    <h3 className="text-4xl md:text-6xl font-manrope font-semibold text-[var(--text-main)] mb-2 leading-[0.9] group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-zinc-500 transition-all duration-500 cursor-default">
                        {client}
                    </h3>
                    <p className="text-xl text-[var(--text-dim)] italic mb-6 font-serif">{title}</p>
                    <div className="px-3 py-1 border border-[var(--border-subtle)] rounded-full text-xs text-[var(--text-dim)] uppercase tracking-widest glass-button cursor-default hover:bg-[var(--bg-elevated)] transition-colors">
                        {category}
                    </div>
                </div>
            </div>
            ...
            <div className={`w-full md:w-1/2 h-[500px] md:h-[600px] relative perspective-midrange group-hover:z-20 ${align === 'right' ? 'md:order-2' : 'md:order-1'}`}>
                <div className="w-full h-full relative transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] transform group-hover:rotate-y-0 group-hover:scale-[1.02] shadow-2xl"
                     style={{ 
                    ...
                    {/* The Image Container */}
                    <div className="absolute inset-0 bg-[var(--bg-card)] overflow-hidden border border-[var(--border-subtle)] shadow-2xl rounded-sm">
